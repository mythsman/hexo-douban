...
function fetchAction(id, action, referer, timeout) {
    let movie = []
    let response = syncFetch("http://mouban.mythsman.com/guest/user_movie?action=" + action + "&id=" + id, referer, timeout)
    if (response === "OFFLINE") {
...
function fetchData(id, referer, timeout) {
    let userResult = syncFetch("http://mouban.mythsman.com/guest/check_user?id=" + id, referer, timeout)
